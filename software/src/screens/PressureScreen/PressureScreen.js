...
// fetch data
  ...
  const fetchTimestamp = async() =>{
    ...
    .catch((err) => {
      ...
      const RenderTable = () => {
        const optimalValues = [
            { name: 'Pressure (Pascals)', value: '1013 hPa' },
          ];
        return (
  ...
  const format_pres = pressure.map(item => {
    const pressure = Number(item.field3);
    return pressure;
  });

  const data = format_date.map((item,index) => ({
    timestamp: format_date[index],
    pressure:  format_pres[index]
  }));

  const renderData = ({item}) => {
    const {timestamp, pressure} = item;
    return(
      <View style={{ flexDirection: 'row', paddingVertical: 10 }}>
        <Text style={{ flex: 1, textAlign: 'left' , fontSize:15}}>{timestamp}</Text>
        <Text style={{ flex: 1, textAlign: 'center', fontSize:17 }}>{pressure === 0 ? "N/A" : `${pressure} hPa`}</Text>
      </View>
    )
  };
                       ...
                       keyExtractor={(item, index)=> {return index.toString(); }}
                        ...
                        ListHeaderComponent={
                          <View style={styles.tableHeader}>
                            ...
                            <Text style={{ flex: 1, textAlign: 'center', fontSize:18 }}>Pressure</Text>
                          </View>
                        }
                        />
                </View>
