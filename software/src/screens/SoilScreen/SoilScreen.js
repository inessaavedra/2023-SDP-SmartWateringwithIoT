...
// fetch data
  ...
  const fetchSoil = async() => {
    fetch('https://api.thingspeak.com/channels/2021316%7D/fields/4.json?api_key=TLDSGLZ1SWUUFAMZ}')
        
    .then((response) => response.json())
    .then((actualData) => {
        console.log(actualData);
        setSoil(actualData.feeds);
    })
    .catch((err) => {
  ...
  const format_soil = soil.map(item => {
    const soil = Number(item.field4);
    return soil;
  ...
  const data = format_date.map((item,index) => ({
    ...
    soil:  format_soil[index]
  }));

  const renderData = ({item}) => {
    const {timestamp, soil} = item;
    return(
      <View style={{ flexDirection: 'row', paddingVertical: 10 }}>
        <Text style={{ flex: 1, textAlign: 'left' , fontSize:15}}>{timestamp}</Text>
        <Text style={{ flex: 1, textAlign: 'center', fontSize:17 }}>{soil === 0 ? "N/A" : `${soil} %`}</Text>
      </View>
    )
  };
                       ...
                       keyExtractor={(item, index)=> {return index.toString(); }}
                        ...
                        ListHeaderComponent={
                          <View style={styles.tableHeader}>
                            ...
                            <Text style={{ flex: 1, textAlign: 'center', fontSize:18 }}>Soil Moisture</Text>
                          </View>
                        }
                        />
                </View>
